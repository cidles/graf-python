...
class PyFeatureStructure:
	...
	def fromFS(fs):
		...
		for f in fs.features():
			...
			if f.isAtomic():
				...
				copy = PyFeatureStructure.fromFS(f.getFSValue())
	...
	def copy(self):
		return PyFeatureStructure.fromFS(self)
	...
	def find(self, name, create=True):
		...
		#findLoop does the real work
		return self.findLoop(name, create)

	def findLoop(self, name, create):
		slash = name.find('/')
		if slash < 0:
			result = self._elements.get(name)
			if result == None and create:
				result = PyFeature(name)
				self.addFeature(result)
			return result
		...
		return fs.findLoop(tail, create)

	
	def add(self, name, value):
		"""
		Creates a new feature and adds it to this C{PyFeatureStructure}'s list of elements
		@param name: C{str}
		@param value: C{str}
		"""
	...
	def subsumes(self, fs2):
		for f1 in self.features():
			...
			if f1.isAtomic():
				if not f2.isAtomic():
					print "f1 isAtomic, fs is not atomic"
					return False
	...
	def unify(self, fs):
		...
		for f1 in fs.features():
			...
			if f2 == None:
				...
				if f1.isAtomic() and f2.isAtomic():
					fVal = f1.getStringValue()
					f2Val = f2.getStringValue()
					if not fVal == f2Val:
						return None
				elif f1.isAtomic() or f2.isAtomic():
